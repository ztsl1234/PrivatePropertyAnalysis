...
Private Property Residential (Non-Landed) Rental Index By localities By Year
...
def processData(fdata, startYear, endYear):
    ...
    #filter for non-landed
    fdata=fdata[fdata['property_type']=="Non-Landed"]
    
    localities=fdata['locality']
    ...
    for locality in allLocalities:
          #filter for locality
          temp=fdata[fdata['locality']==locality]
          meanIndexByLocalityYear[locality]=temp.groupby(['year'])[['index']].mean()
...
def displayLineChart(chartData):
    ...
    title = "Private Property Rental Index of Non-landed Residential Properties by Locality"
    titlelen = len(title)
...
rentalIndexByYear=processData(data1, startYear, endYear)
displayLineChart(rentalIndexByYear)
